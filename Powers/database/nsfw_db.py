...
class NSFWSettings(MongoDB):
    ...
    def set_mode(self, chat_id, mode):
        if mode not in ("off","soft","normal","strict"):
            return False
        self.update({"chat_id":chat_id},{"$set":{"chat_id":chat_id,"mode":mode}},upsert=True)
        return True
...
class NSFWApprove(MongoDB):
    ...
    def approve(self, chat_id, user_id, approved_by):
        if self.is_approved(chat_id, user_id):
            return False
        self.insert_one({"chat_id":chat_id,"user_id":user_id,"approved_by":approved_by,"approved_at":datetime.utcnow()})
        return True
...
class NSFWViolations(MongoDB):
    ...
    def add_violation(self, chat_id, user_id, category):
        existing = self.find_one({"chat_id":chat_id,"user_id":user_id,"category":category})
        if existing:
            self.update({"chat_id":chat_id,"user_id":user_id,"category":category},{"$inc":{"count":1},"$set":{"last_seen":datetime.utcnow()}})
        else:
            self.insert_one({"chat_id":chat_id,"user_id":user_id,"category":category,"count":1,"last_seen":datetime.utcnow()})
