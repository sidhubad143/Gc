...
async def lock_del_mess(c: Gojo, m: Message):
    ...
    elif any(chat_locks.get("anti_fwd", {}).values()) and (m.forward_from or m.forward_from_chat):
        if all(chat_locks["anti_fwd"].values()):
            await delete_messages(c, m)
            return
        elif chat_locks["anti_fwd"].get("user") and not m.forward_from_chat:
            await delete_messages(c, m)
            return
        elif chat_locks["anti_fwd"].get("chat") and m.forward_from_chat:
            await delete_messages(c, m)
            return
